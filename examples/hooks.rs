use amd64_emu::{Engine, EngineMode, Register, Permission, HookType};
...
fn main() {
    ...
    let code = vec![
        ...
        0x48, 0x89, 0x04, 0x25, 0x08, 0x20, 0x00, 0x00,
        0x48, 0x8B, 0x04, 0x25, 0x00, 0x20, 0x00, 0x00,
        0x48, 0x03, 0x04, 0x25, 0x08, 0x20, 0x00, 0x00,
    ];
    ...
    engine.hook_add(
        ...
        move |cpu, addr, size| {
            ...
            *count += 1;
            println!("[CODE] Executing instruction at {:#x} (size: {} bytes)", addr, size);
    ...
    println!("Code hook executed {} times", *code_hook_count.lock().unwrap());
