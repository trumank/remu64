...
fn main() {
    ...
    // Step through instruction by instruction for debugging
    engine.set_trace_level(remu64::tracer::TraceLevel::Instructions);
    ...
    println!("Before: CF = {}", engine.flags_read().contains(remu64::cpu::Flags::CF));
    ...
    println!("After MOVs (before SHLD): EAX = {:#x}, EBX = {:#x}", 
            engine.reg_read(Register::EAX), 
            engine.reg_read(Register::EBX));
    
    // Now execute SHLD
    engine.emu_start(0x100a, 0x1000 + code.len() as u64, 0, 0).unwrap();
    
    println!("After SHLD: EAX = {:#x}, EBX = {:#x}", 
            engine.reg_read(Register::EAX), 
            engine.reg_read(Register::EBX));
    
    // The MSB (bit 31) of 0x80000000 should be shifted into CF
    println!("After: CF = {}", engine.flags_read().contains(remu64::cpu::Flags::CF));
    ...
    engine.emu_start(0x2000, 0x2000 + code2.len() as u64, 0, 0).unwrap();
