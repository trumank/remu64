...
fn test_cmpps_equal() {
    ...
    emu.xmm_write(Register::XMM0, xmm0_val);
    emu.xmm_write(Register::XMM1, xmm1_val);
...
fn test_comiss_equal() {
    ...
    assert!(flags.contains(remu64::Flags::ZF)); // Zero flag set for equal
    assert!(!flags.contains(remu64::Flags::CF)); // Carry flag clear
    assert!(!flags.contains(remu64::Flags::PF)); // Parity flag clear
}
...
fn test_comiss_less_than() {
    ...
    let flags = emu.flags_read();
    assert!(!flags.contains(remu64::Flags::ZF)); // Zero flag clear
    assert!(flags.contains(remu64::Flags::CF)); // Carry flag set for less than
    assert!(!flags.contains(remu64::Flags::PF)); // Parity flag clear
}
...
fn test_comiss_greater_than() {
    ...
    let flags = emu.flags_read();
    assert!(!flags.contains(remu64::Flags::ZF)); // Zero flag clear
    assert!(!flags.contains(remu64::Flags::CF)); // Carry flag clear for greater than
    assert!(!flags.contains(remu64::Flags::PF)); // Parity flag clear
}
...
fn test_ucomiss_nan() {
    ...
    let flags = emu.flags_read();
    assert!(flags.contains(remu64::Flags::ZF)); // Zero flag set for unordered
    assert!(flags.contains(remu64::Flags::CF)); // Carry flag set for unordered
    assert!(flags.contains(remu64::Flags::PF)); // Parity flag set for unordered
}
