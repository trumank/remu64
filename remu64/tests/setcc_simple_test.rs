...
fn test_seta_setae_setb() {
    ...
    assert_eq!(
        engine.reg_read(Register::CL),
        1,
        "SETA should set for 5 > 3"
    );
    assert_eq!(
        engine.reg_read(Register::DL),
        1,
        "SETAE should set for 5 >= 3"
    );
    assert_eq!(
        engine.reg_read(Register::SIL),
        0,
        "SETB should not set for 5 > 3"
    );
}
...
fn test_setg_setge_setl_setle() {
    ...
    assert_eq!(
        engine.reg_read(Register::CL),
        1,
        "SETG should set for 5 > 3"
    );
    assert_eq!(
        engine.reg_read(Register::DL),
        1,
        "SETGE should set for 5 > 3"
    );
    assert_eq!(
        engine.reg_read(Register::SIL),
        0,
        "SETL should not set for 5 > 3"
    );
    assert_eq!(
        engine.reg_read(Register::DIL),
        0,
        "SETLE should not set for 5 > 3"
    );
}
...
fn test_sets_setns() {
    ...
    assert_eq!(
        engine.reg_read(Register::CL),
        1,
        "SETS should set when SF=1"
    );
    assert_eq!(
        engine.reg_read(Register::DL),
        0,
        "SETNS should not set when SF=1"
    );
}
...
fn test_seto_setno() {
    ...
    assert_eq!(
        engine.reg_read(Register::CL),
        1,
        "SETO should set when OF=1"
    );
    assert_eq!(
        engine.reg_read(Register::DL),
        0,
        "SETNO should not set when OF=1"
    );
}
...
fn test_setp_setnp() {
    ...
    assert_eq!(
        engine.reg_read(Register::CL),
        1,
        "SETP should set when PF=1 (even parity)"
    );
    assert_eq!(
        engine.reg_read(Register::DL),
        0,
        "SETNP should not set when PF=1"
    );
}
