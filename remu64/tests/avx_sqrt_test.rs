...
fn test_vsqrtpd_xmm_register() {
    ...
    // 4.0, 9.0
    let test_data: u128 = 
        (4.0f64.to_bits() as u128) |
        ((9.0f64.to_bits() as u128) << 64);
...
fn test_vsqrtpd_ymm_register() {
    ...
    let upper_data: u128 = 
        (16.0f64.to_bits() as u128) |
        ((25.0f64.to_bits() as u128) << 64);
...
fn test_vsqrtps_memory() {
    ...
    for (i, &value) in test_values.iter().enumerate() {
        ...
        for (j, &byte) in bytes.iter().enumerate() {
            engine.memory.write(memory_addr + (i * 4) as u64 + j as u64, &[byte]).unwrap();
        }
    }
...
fn test_vsqrtpd_memory() {
    ...
    for (i, &value) in test_values.iter().enumerate() {
        let bytes = value.to_bits().to_le_bytes();
        for (j, &byte) in bytes.iter().enumerate() {
            engine.memory.write(memory_addr + (i * 8) as u64 + j as u64, &[byte]).unwrap();
        }
    }
...
fn test_vsqrtpd_special_values() {
    ...
    let test_data: u128 = 
        ...
        ((f64::INFINITY.to_bits() as u128) << 64);
