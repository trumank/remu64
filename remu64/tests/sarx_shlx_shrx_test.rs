...
fn test_sarx_basic() {
    ...
    let code = vec![
        ...
        0xC4, 0xE2, 0xF3, 0xF7, 0xD8,              // sarx rbx, rax, rcx (VEX.LZ.F3.0F38.W1)
    ];
    
    engine.memory.write(0x1000, &code).unwrap();
    engine.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 0).unwrap();
    
    assert_eq!(engine.reg_read(Register::RBX), 0xFFFFFFFFFFFFFFFE, "SARX should sign-extend");
}
...
fn test_sarx_positive() {
    ...
    let code = vec![
        ...
        0xC4, 0xE2, 0xEB, 0xF7, 0xD8,              // sarx rbx, rax, rdx
    ];
    
    engine.memory.write(0x1000, &code).unwrap();
    engine.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 0).unwrap();
    
    assert_eq!(engine.reg_read(Register::RBX), 0x08, "SARX on positive should work like SHR");
}
...
fn test_sarx_no_flags() {
    ...
    let code = vec![
        ...
        0xC4, 0xE2, 0xEB, 0xF7, 0xD9,              // sarx rbx, rcx, rdx
    ];
    
    engine.memory.write(0x1000, &code).unwrap();
    engine.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 0).unwrap();
    
    assert!(engine.cpu.rflags.contains(remu64::cpu::Flags::ZF), "ZF should remain set");
    assert_eq!(engine.reg_read(Register::RBX), 0xFFFFFFFFFFFFFFFF, "SARX should sign-extend -1");
}
...
fn test_sarx_32bit() {
    ...
    let code = vec![
        ...
        0xC4, 0xE2, 0x73, 0xF7, 0xD8,              // sarx ebx, eax, ecx (32-bit)
    ];
    
    engine.memory.write(0x1000, &code).unwrap();
    engine.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 0).unwrap();
    
    assert_eq!(engine.reg_read(Register::RBX) & 0xFFFFFFFF, 0xFFFFFFFF, 
...
fn test_shlx_basic() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0x10, "SHLX should shift left");
}
...
fn test_shlx_overflow() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0, "SHLX should shift out MSB");
}
...
fn test_shrx_basic() {
    ...
    let code = vec![
        0x48, 0xC7, 0xC0, 0x80, 0x00, 0x00, 0x00,  // mov rax, 0x80
        0x48, 0xC7, 0xC1, 0x04, 0x00, 0x00, 0x00,  // mov rcx, 4
        0xC4, 0xE2, 0xF3, 0xF5, 0xD8,              // shrx rbx, rax, rcx
    ];
    
    engine.memory.write(0x1000, &code).unwrap();
    engine.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 0).unwrap();
    
    assert_eq!(engine.reg_read(Register::RBX), 0x08, "SHRX should shift right");
}
...
fn test_shrx_no_sign_extend() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0x4000000000000000, 
...
fn test_shift_modulo() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0x02, "Shift by 65 should be same as shift by 1");
}
...
fn test_32bit_shifts() {
    ...
    let code = vec![
        ...
        0x48, 0xC7, 0xC1, 0x04, 0x00, 0x00, 0x00,  // mov rcx, 4
        0xC4, 0xE2, 0x73, 0xF5, 0xE8,              // shrx ebp, eax, ecx
        
        // SARX 32-bit with sign bit
        0x48, 0xC7, 0xC0, 0x00, 0x00, 0x00, 0x80,  // mov rax, 0x80000000
        0x48, 0xC7, 0xC1, 0x04, 0x00, 0x00, 0x00,  // mov rcx, 4
        0xC4, 0xE2, 0x73, 0xF7, 0xF0,              // sarx esi, eax, ecx
    ];
    
    engine.memory.write(0x1000, &code).unwrap();
    engine.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 0).unwrap();
    
    assert_eq!(engine.reg_read(Register::RBX) & 0xFFFFFFFF, 0x100, "32-bit SHLX");
    assert_eq!(engine.reg_read(Register::RBP) & 0xFFFFFFFF, 0x10, "32-bit SHRX");
    assert_eq!(engine.reg_read(Register::RSI) & 0xFFFFFFFF, 0xF8000000, "32-bit SARX");
}
...
fn test_zero_shift_count() {
    ...
    let code = vec![
        ...
        0xC4, 0xE2, 0xF1, 0xF7, 0xD8,              // shlx rbx, rax, rcx
        0xC4, 0xE2, 0xF3, 0xF5, 0xE8,              // shrx rbp, rax, rcx
        0xC4, 0xE2, 0xF3, 0xF7, 0xF0,              // sarx rsi, rax, rcx
    ];
