...
fn test_bzhi_32bit_basic() {
    ...
    assert!(
        ...
        "CF should be cleared"
    );
    assert!(
        !engine.cpu.rflags.contains(remu64::Flags::OF),
        "OF should be cleared"
    );
    assert!(
        !engine.cpu.rflags.contains(remu64::Flags::SF),
        "SF should be clear"
    );
    assert!(
...
fn test_bzhi_32bit_negative_result() {
    ...
    assert!(
        engine.cpu.rflags.contains(remu64::Flags::SF),
        "SF should be set for negative result"
    );
    assert!(
        !engine.cpu.rflags.contains(remu64::Flags::ZF),
        "ZF should be clear"
...
fn test_bzhi_64bit_full_width() {
    ...
    let code = vec![
        0x48, 0xB8, 0x10, 0x32, 0x54, 0x76, 0x98, 0xBA, 0xDC,
        0xFE, // mov rax, 0xFEDCBA9876543210
...
fn test_bzhi_64bit_zero_result() {
    ...
    let code = vec![
        ...
        0xFF, // mov rax, 0xFF00000000000000
        0x48, 0xC7, 0xC1, 0x20, 0x00, 0x00, 0x00, // mov rcx, 32
        0xC4, 0xE2, 0xF0, 0xF5, 0xD8, // bzhi rbx, rax, rcx
    ];
