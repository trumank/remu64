...
fn test_mulx_32bit_basic() {
    ...
    assert_eq!(
        engine.cpu.read_reg(Register::RBX) as u32,
        0,
        "High bits should be 0"
    );
    assert_eq!(
        engine.cpu.read_reg(Register::RDX) as u32,
        0xF,
        "Low bits should be 0xF in EDX (iced-x86 quirk)"
    );
    assert_eq!(
        engine.cpu.rflags.bits(),
        initial_flags.bits(),
        "MULX should not modify flags"
    );
}
...
fn test_mulx_64bit_basic() {
    ...
    assert_eq!(
        engine.cpu.read_reg(Register::RBX),
        0,
        "High bits should be 0"
    );
    assert_eq!(
        engine.cpu.read_reg(Register::RDX),
        0xF,
        "Low bits should be 0xF in RDX"
    );
    assert_eq!(
        engine.cpu.rflags.bits(),
        initial_flags.bits(),
...
fn test_mulx_64bit_large() {
    ...
    let code = vec![
        0x48, 0xBA, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x10, // mov rdx, 0x1000000000000000
...
fn test_mulx_64bit_mixed() {
    ...
    let code = vec![
        0x48, 0xBA, 0xF0, 0xDE, 0xBC, 0x9A, 0x78, 0x56, 0x34,
        0x12, // mov rdx, 0x123456789ABCDEF0
...
fn test_mulx_preserves_flags_cf_set() {
    ...
    assert_eq!(
        engine.cpu.read_reg(Register::RDX) as u32,
        6,
        "Low bits should be 6 in EDX"
    );
    assert!(
        engine.cpu.rflags.contains(remu64::Flags::CF),
        "CF should remain set"
    );
}
...
fn test_mulx_preserves_flags_zf_sf_set() {
    ...
    assert_eq!(
        engine.cpu.read_reg(Register::RDX) as u32,
        20,
        "Low bits should be 20 in EDX"
    );
    assert!(
        engine.cpu.rflags.contains(remu64::Flags::ZF),
