...
fn test_blsmsk_basic() {
    ...
    assert_eq!(
        ...
        0x03,
        "BLSMSK should create mask up to lowest set bit"
    );
    assert!(
...
fn test_blsmsk_zero() {
    ...
    assert_eq!(
        engine.reg_read(Register::RAX),
        0xFFFFFFFFFFFFFFFF,
        "BLSMSK of zero should be all 1s"
    );
    assert!(
        engine.cpu.rflags.contains(remu64::cpu::Flags::ZF),
...
fn test_blsmsk_powers_of_two() {
    ...
    let code = vec![
        ...
        0xC4, 0xE2, 0xF8, 0xF3, 0xD0, // blsmsk rax, rax (writes back to rax)
    ...
    assert_eq!(
        ...
        0x01,
        "BLSMSK of 0x01 should be 0x01"
    );
    assert_eq!(
        engine.reg_read(Register::RDX),
        0x0F,
        "BLSMSK of 0x08 should be 0x0F"
    );
    assert_eq!(
        engine.reg_read(Register::RCX),
        0xFF,
        "BLSMSK of 0x80 should be 0xFF"
    );
}
...
fn test_blsmsk_multiple_bits() {
    ...
    assert_eq!(
        engine.reg_read(Register::RBX),
        0x01,
        "BLSMSK of 0xFF should be 0x01"
    );
    assert_eq!(
        engine.reg_read(Register::RDX),
        0x07,
        "BLSMSK of 0x1C should be 0x07"
    );
    assert_eq!(
        engine.reg_read(Register::RCX),
        0x1F,
        "BLSMSK of 0xF0 should be 0x1F"
    );
}
...
fn test_blsmsk_32bit() {
    ...
    let code = vec![
        ...
        0xC4, 0xE2, 0x68, 0xF3, 0xD0, // blsmsk edx, eax (32-bit version)
    ];
    ...
    assert_eq!(
        engine.reg_read(Register::EDX),
        0x1F,
        "32-bit BLSMSK of 0x30 should be 0x1F"
    );
    assert_eq!(
        engine.reg_read(Register::RDX),
        0x1F,
