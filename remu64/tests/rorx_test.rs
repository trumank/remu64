...
fn test_rorx_no_flags() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0xFF00000000000000, "RORX result should be correct");
}
...
fn test_rorx_32bit() {
    ...
    assert_eq!(engine.reg_read(Register::RBX) & 0xFFFFFFFF, 0x56781234, "32-bit RORX should rotate by 16");
}
...
fn test_rorx_wrap_around() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0x0000000000000001, "RORX by 63 should wrap MSB to LSB");
}
...
fn test_rorx_zero_count() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0x42, "RORX by 0 should not change value");
}
...
fn test_rorx_modulo_count() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0x01, "RORX by 65 should be same as by 1");
}
...
fn test_rorx_memory_source() {
    ...
    engine.memory.write(0x2000, &0x123456789ABCDEF0u64.to_le_bytes()).unwrap();
    
    // Test RORX with memory source
    let code = vec![
        0xC4, 0xE3, 0xFB, 0xF0, 0x1C, 0x25, 0x00, 0x20, 0x00, 0x00, 0x08,  // rorx rbx, qword ptr [0x2000], 8
    ];
    
    engine.memory.write(0x1000, &code).unwrap();
    engine.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 0).unwrap();
    
    assert_eq!(engine.reg_read(Register::RBX), 0xF0123456789ABCDE, "RORX from memory should work");
}
