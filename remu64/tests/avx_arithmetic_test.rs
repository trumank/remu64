...
#[test]
fn test_vaddps_xmm() {
    let mut engine = Engine::new(0x1000, 0x2000);
    ...
    engine.cpu.write_xmm(Register::XMM1, xmm1_data);
    engine.cpu.write_xmm(Register::XMM2, xmm2_data);

    // vaddps xmm0, xmm1, xmm2
    let mut asm = CodeAssembler::new(64).unwrap();
    asm.vaddps(xmm0, xmm1, xmm2).unwrap();
    let code = asm.assemble(0x1000).unwrap();
    
    engine.load_code(&code, 0x1000);
    engine.execute_at(0x1000).unwrap();

    // Expected: XMM0 = [6.0, 8.0, 10.0, 12.0]
    let result = engine.cpu.read_xmm(Register::XMM0);
...
fn test_vsubps_xmm() {
    ...
    let mut asm = CodeAssembler::new(64).unwrap();
    asm.vsubps(xmm0, xmm1, xmm2).unwrap();
    let code = asm.assemble(0x1000).unwrap();
...
fn test_vmulps_xmm() {
    ...
    let mut asm = CodeAssembler::new(64).unwrap();
    asm.vmulps(xmm0, xmm1, xmm2).unwrap();
    let code = asm.assemble(0x1000).unwrap();
...
fn test_vdivps_xmm() {
    ...
    let mut asm = CodeAssembler::new(64).unwrap();
    asm.vdivps(xmm0, xmm1, xmm2).unwrap();
    let code = asm.assemble(0x1000).unwrap();
...
fn test_vaddpd_xmm() {
    ...
    let mut asm = CodeAssembler::new(64).unwrap();
    asm.vaddpd(xmm0, xmm1, xmm2).unwrap();
    let code = asm.assemble(0x1000).unwrap();
...
fn test_vsubpd_xmm() {
    ...
    let mut asm = CodeAssembler::new(64).unwrap();
    asm.vsubpd(xmm0, xmm1, xmm2).unwrap();
    let code = asm.assemble(0x1000).unwrap();
...
fn test_vaddps_ymm() {
    ...
    engine.cpu.write_ymm(Register::YMM1, [ymm1_low, ymm1_high]);
    engine.cpu.write_ymm(Register::YMM2, [ymm2_low, ymm2_high]);

    // vaddps ymm0, ymm1, ymm2
    let mut asm = CodeAssembler::new(64).unwrap();
    asm.vaddps(ymm0, ymm1, ymm2).unwrap();
    let code = asm.assemble(0x1000).unwrap();
    ...
    let result = engine.cpu.read_ymm(Register::YMM0);
