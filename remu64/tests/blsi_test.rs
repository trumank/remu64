...
fn test_blsi_basic() {
    ...
    assert_eq!(
        ...
        0x02,
        "BLSI should isolate lowest set bit"
    );
    assert!(
        !engine.cpu.rflags.contains(remu64::cpu::Flags::ZF),
        "ZF should be clear for non-zero input"
    );
    assert!(
...
fn test_blsi_zero() {
    ...
    assert_eq!(
        engine.reg_read(Register::RBX),
        0,
        "BLSI of zero should be zero"
    );
    assert!(
        engine.cpu.rflags.contains(remu64::cpu::Flags::ZF),
        "ZF should be set for zero input"
    );
    assert!(
        !engine.cpu.rflags.contains(remu64::cpu::Flags::CF),
...
fn test_blsi_powers_of_two() {
    ...
    assert_eq!(
        engine.reg_read(Register::RBX),
        0x01,
        "BLSI of 0x01 should be 0x01"
    );
    assert_eq!(
        engine.reg_read(Register::RDX),
        0x08,
        "BLSI of 0x08 should be 0x08"
    );
    assert_eq!(
        engine.reg_read(Register::RCX),
        0x80,
        "BLSI of 0x80 should be 0x80"
    );
}
...
fn test_blsi_multiple_bits() {
    ...
    assert_eq!(
        engine.reg_read(Register::RBX),
        0x01,
        "BLSI of 0xFF should be 0x01"
    );
    assert_eq!(
        engine.reg_read(Register::RDX),
        0x04,
        "BLSI of 0x1C should be 0x04"
    );
    assert_eq!(
        engine.reg_read(Register::RCX),
        0x10,
        "BLSI of 0xF0 should be 0x10"
    );
}
...
fn test_blsi_32bit() {
    ...
    assert_eq!(
        ...
        0x10,
        "32-bit BLSI of 0x30 should be 0x10"
    );
    assert_eq!(
        engine.reg_read(Register::RBX),
        0x10,
...
fn test_blsi_alternating_pattern() {
    ...
    assert_eq!(
        engine.reg_read(Register::RBX),
        0x0002,
        "BLSI of 0xAAAA should be 0x0002"
    );
    assert_eq!(
        engine.reg_read(Register::RDX),
        0x0001,
        "BLSI of 0x5555 should be 0x0001"
    );
}
...
fn test_blsi_memory_operand() {
    ...
    let code = vec![
        0xC4, 0xE2, 0xE0, 0xF3, 0x1C, 0x25, 0x00, 0x20, 0x00,
        0x00, // blsi rbx, qword ptr [0x2000] (VEX.LZ.0F38.F3 /3)
    ];
    ...
    assert_eq!(
        engine.reg_read(Register::RBX),
        0x08,
        "BLSI of 0x48 from memory should be 0x08"
    );
}
...
fn test_blsi_sign_flag() {
    ...
    assert_eq!(
        engine.reg_read(Register::EBX),
        0x80000000,
        "BLSI of 0x80000000 should be 0x80000000"
    );
    assert!(
        engine.cpu.rflags.contains(remu64::cpu::Flags::SF),
        "SF should be set when result has MSB set"
    );
}
