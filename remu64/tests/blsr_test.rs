...
fn test_blsr_basic() {
    ...
    assert_eq!(engine.reg_read(Register::RAX), 0x10, "BLSR should reset lowest set bit");
    assert!(!engine.cpu.rflags.contains(remu64::cpu::Flags::ZF), "ZF should be clear for non-zero result");
...
fn test_blsr_zero() {
    ...
    assert_eq!(engine.reg_read(Register::RAX), 0, "BLSR of zero should be zero");
    assert!(engine.cpu.rflags.contains(remu64::cpu::Flags::ZF), "ZF should be set for zero result");
...
fn test_blsr_powers_of_two() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0x00, "BLSR of 0x01 should be 0x00");
    assert_eq!(engine.reg_read(Register::RDX), 0x00, "BLSR of 0x08 should be 0x00");
    assert_eq!(engine.reg_read(Register::RCX), 0x00, "BLSR of 0x80 should be 0x00");
}
...
fn test_blsr_multiple_bits() {
    ...
    assert_eq!(engine.reg_read(Register::RBX), 0xFE, "BLSR of 0xFF should be 0xFE");
    assert_eq!(engine.reg_read(Register::RDX), 0x18, "BLSR of 0x1C should be 0x18");
    assert_eq!(engine.reg_read(Register::RCX), 0xE0, "BLSR of 0xF0 should be 0xE0");
}
...
fn test_blsr_consecutive_application() {
    ...
    assert_eq!(engine.reg_read(Register::RAX), 0x00, "Three applications of BLSR on 0x07 should result in 0x00");
    assert!(engine.cpu.rflags.contains(remu64::cpu::Flags::ZF), "ZF should be set when result is zero");
}
...
fn test_blsr_32bit() {
    ...
    assert_eq!(engine.reg_read(Register::EAX), 0x20, "32-bit BLSR of 0x30 should be 0x20");
    assert_eq!(engine.reg_read(Register::RAX), 0x20, "Upper 32 bits should be cleared");
}
...
fn test_blsr_large_values() {
    ...
    assert_eq!(engine.reg_read(Register::RDX), 0xFFFFFFFFFFFFFFE0, 
