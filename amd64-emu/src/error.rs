...
impl fmt::Display for EmulatorError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            EmulatorError::UnmappedMemory(addr) => write!(f, "Memory not mapped at address: {:#x}", addr),
            EmulatorError::PermissionDenied(addr) => write!(f, "Permission denied for operation at address: {:#x}", addr),
            EmulatorError::InvalidInstruction(addr) => write!(f, "Invalid instruction at address: {:#x}", addr),
            EmulatorError::UnsupportedInstruction(instr) => write!(f, "Unsupported instruction: {}", instr),
