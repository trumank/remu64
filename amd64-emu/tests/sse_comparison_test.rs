...
fn test_cmpps_equal() {
    ...
    emu.cpu.write_xmm(Register::XMM0, xmm0_val);
    emu.cpu.write_xmm(Register::XMM1, xmm1_val);
    ...
    emu.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 0).unwrap();
    
    // Check result - should be [0xFFFFFFFF, 0, 0xFFFFFFFF, 0]
    let result = emu.cpu.read_xmm(Register::XMM0);
