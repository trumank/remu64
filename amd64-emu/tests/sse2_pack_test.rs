...
fn test_packsswb() {
    ...
    emu.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 3).unwrap(); // packsswb
    ...
    assert_eq!(result, expected, "PACKSSWB failed: got {:#034x}, expected {:#034x}", result, expected);
}
...
fn test_packuswb() {
    ...
    // Execute instructions
    emu.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 3).unwrap(); // packuswb
    ...
    assert_eq!(result, expected, "PACKUSWB failed: got {:#034x}, expected {:#034x}", result, expected);
}
...
fn test_packssdw() {
    ...
    // Execute instructions
    emu.emu_start(0x1000, 0x1000 + code.len() as u64, 0, 3).unwrap(); // packssdw
    ...
    assert_eq!(result, expected, "PACKSSDW failed: got {:#034x}, expected {:#034x}", result, expected);
}
...
fn test_pack_memory_operand() {
    ...
    let expected = 
        0x10u128 | (0x20u128 << 8) | (0x30u128 << 16) | (0x40u128 << 24) |
        (0x50u128 << 32) | (0x60u128 << 40) | (0x70u128 << 48) | (0x7Fu128 << 56) |
        (0x01u128 << 64) | (0x02u128 << 72) | (0x03u128 << 80) | (0x04u128 << 88) |
        (0x05u128 << 96) | (0x06u128 << 104) | (0x07u128 << 112) | (0x08u128 << 120);
    
    assert_eq!(result, expected, "PACKSSWB with memory failed: got {:#034x}, expected {:#034x}", result, expected);
}