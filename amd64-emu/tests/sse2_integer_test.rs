...
fn test_paddb() {
    ...
    // Check result (data starts at 0x1020, result written to offset 0x30 = 0x1040)
    let mut result = vec![0u8; 16];
    engine.mem_read(0x1040, &mut result).unwrap();
    
    // Debug: Check what XMM0 contains
    let xmm0_value = engine.xmm_read(Register::XMM0);
    println!("XMM0 after PADDB: {:032x}", xmm0_value);
