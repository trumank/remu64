...
fn test_pmaddwd() {
    ...
    let code = vec![
        ...
        0x66, 0x0F, 0x6F, 0x4C, 0x24, 0x10, // pmaddwd xmm0, xmm1
        0x66, 0x0F, 0xF5, 0xC1,
    ];
    ...
    let expected = 18u128 | (3u128 << 32) | (8u128 << 64) | (50u128 << 96);

    assert_eq!(
        result, expected,
        "PMADDWD failed: got {:#034x}, expected {:#034x}",
        result, expected
    );
}
...
fn test_pmaddwd_overflow() {
    ...
    let expected =
        0x1FFFCu128 | (0xFFFE0000u128 << 32) | (0x18000u128 << 64) | (0xFFFFFFFFu128 << 96);

    assert_eq!(
        result, expected,
        "PMADDWD overflow test failed: got {:#034x}, expected {:#034x}",
        result, expected
    );
}
...
fn test_pmaddwd_memory() {
    ...
    let code = vec![
        // movdqa xmm0, [rsp]
        0x66, 0x0F, 0x6F, 0x04, 0x24, // pmaddwd xmm0, [rsp+16]
    ...
    let expected = 3u128 | (7u128 << 32) | (11u128 << 64) | (15u128 << 96);

    assert_eq!(
        result, expected,
        "PMADDWD memory test failed: got {:#034x}, expected {:#034x}",
        result, expected
    );
}
...
fn test_pmaddwd_negative() {
    ...
    assert_eq!(
        result, expected,
        "PMADDWD negative test failed: got {:#034x}, expected {:#034x}",
        result, expected
    );
}
