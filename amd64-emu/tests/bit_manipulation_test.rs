...
fn test_bt_instruction() {
    ...
    assert!(engine.cpu.rflags.contains(amd64_emu::Flags::CF));
    ...
    engine.emu_start(0x1000, 0x1000 + code2.len() as u64, 0, 0).unwrap();
...
fn test_bsf_instruction() {
    ...
    assert!(!engine.cpu.rflags.contains(amd64_emu::Flags::ZF));
}
